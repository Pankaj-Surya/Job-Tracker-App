...
import type { Job, JobLead } from './types';
...
export default function App() {
  ...
  const fetchFreshJobs = async () => {
    ...
    setLeadError(null);
    try {
      const response = await fetch('/api/job-leads');
      const contentType = response.headers.get('content-type') || '';
      if (!contentType.includes('application/json')) {
        throw new Error('Job API is not running here. Use Vercel deployment or `vercel dev` with Adzuna env vars for live jobs.');
      }
